...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        // 创建 DubboExporter 对象，并添加到 exporterMap中
        // export service.
    ...
    private ExchangeServer createServer(URL url) {
        ...
        // 校验 server 的Dubbo SPI 扩展是否存在
        String str = url.getParameter(Constants.SERVER_KEY, Constants.DEFAULT_REMOTING_SERVER);
        if (str != null && str.length() > 0 && !ExtensionLoader.getExtensionLoader(Transporter.class).hasExtension(str))
            throw new RpcException("Unsupported server type: " + str + ", url: " + url);

        // 设置编解码器为 Dubbo : DubboCountCodec
    ...
    private ExchangeClient getSharedClient(URL url) {
        // 从集合中查找ReferenceCountExchangeClient对象
        String key = url.getAddress();
        ReferenceCountExchangeClient client = referenceClientMap.get(key);
        if (client != null) {
            // 若未关闭，增加指向该Client的数量并返回
            if (!client.isClosed()) {
    ...
    private ExchangeClient initClient(URL url) {
        ...
        // 校验client的Duboo SPI 扩展是否存在
        // client type setting.
        String str = url.getParameter(Constants.CLIENT_KEY, url.getParameter(Constants.SERVER_KEY, Constants.DEFAULT_REMOTING_CLIENT));

        // 设置编解码器为dubbo DubboCountCodec
        url = url.addParameter(Constants.CODEC_KEY, DubboCodec.NAME);
        // enable heartbeat by default
        url = url.addParameterIfAbsent(Constants.HEARTBEAT_KEY, String.valueOf(Constants.DEFAULT_HEARTBEAT));
